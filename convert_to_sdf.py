...
def converter(path, target_path, dataset, algo, process=20, debug=False):
    ...
    x_y_name_dict = {'dataset': ('smiles','pka')}
    x_y_name_dict['bpka_30000'] = ('smiles', 'pka')
    x_y_name_dict['pka_30000'] = ('smiles', 'pka')
...
# run: python convert_to_sdf.py --dataset [name] --process  --algo MMFF
if __name__ == "__main__":
    ...
    #print("./data/%s/%s.csv"%args.dataset, "./data/%s/%s.sdf"%(args.dataset, args.dataset), args.dataset, args.algo, process=args.process)
    converter("./data/%s/%s.csv"%(args.dataset, args.dataset), "./data/%s/%s.sdf"%(args.dataset, args.dataset), args.dataset, args.algo, process=args.process)
