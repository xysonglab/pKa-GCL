...
class MoleculeDataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        return self.a2a_graphs[idx], self.e2a_graphs[idx], self.e2e_graphs[idx],self.smile_list[idx], self.labels[idx]
    ...
    def load_data(self):
        ...
        if self.has_cache():
            ...
            with open(file_name, 'rb') as f:
                data_mols, smile_list,data_Y = pickle.load(f)

            for mol,smile, y in tqdm(zip(data_mols,smile_list, data_Y)):
...
class Molecule2DView(MoleculeDataset):
    ...
    def build_graph(self, mol):
        ...
        if len(atom_features) != num_atoms:
            print(f"Warning:Atom features mismatch in mocular. Expected {num_atoms}, got {len(atom_features)}")
            return None
