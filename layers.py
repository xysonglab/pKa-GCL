...
class Angle2DConv(nn.Layer):
    ...
    def recv_func(self, msg):
        return msg.reduce(msg["h"], pool_type="max")
...
class MaxPooling(nn.Layer):
    def __init__(self, in_dim, dropout):
        ...
        self.pool = pgl.nn.GraphPool(pool_type='max')  # Changed to max pooling
    ...
    def forward(self, graph, node_feat, graph_feat):
        ...
        context = self.pool(graph, node_h * node_a)  # Apply max pooling here
        graph_h, _ = self.gru(context, graph_feat)  # Pass the pooled context to GRU
        return graph_h
...
class OutputLayer(nn.Layer):
    def __init__(self, in_dim, hidden_dims, num_pool, dropout_pool):
        ...
        for _ in range(num_pool):
            self.attn_pool_layers.append(MaxPooling(in_dim, dropout=dropout_pool))
