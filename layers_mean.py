...
class Angle2DConv(nn.Layer):
    ...
    def recv_func(self, msg):
        return msg.reduce(msg["h"], pool_type="mean")
...
class AveragePooling(nn.Layer):
    def __init__(self, in_dim, dropout):
        ...
        self.pool = pgl.nn.GraphPool(pool_type='mean')  # Changed to average pooling
    ...
    def forward(self, graph, node_feat, graph_feat):
        ...
        context = self.pool(graph, node_h * node_a)  # Apply average pooling here
...
class OutputLayer(nn.Layer):
    def __init__(self, in_dim, hidden_dims, num_pool, dropout_pool):
        ...
        for _ in range(num_pool):
            self.attn_pool_layers.append(AveragePooling(in_dim, dropout=dropout_pool))
