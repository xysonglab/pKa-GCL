...
from ATTData_all import PkaHybridDataset  # Replace with actual dataset class path
...
class ATT(torch.nn.Module):
    def __init__(self, n_heads=4, edge_dim=12, num_features=33, embedding_size=128, self_attention=False,
...
class ATT_fine(torch.nn.Module):
    def __init__(self, pretrain_net):
        ...
        self.linear2 = Linear(self.class_embedding_size, 1)
...
# Data loading
def load_data(root, filename, batch_size=32, train_ratio=0.8, test_ratio=0.1):
    """
    ...
    """
    dataset = PkaHybridDataset(root=root, filename=filename)
...
# Training function
# Added functionality to save predictions in train_model function
def train_model(model, train_loader, val_loader, test_loader, epochs=50, lr=0.001):
    ...
    for epoch in range(epochs):
        ...
        for data in train_loader:
            ...
            target = data.y.float().view(-1)  # Flatten target
    ...
    pd.DataFrame(history).to_csv('training_log_23d.csv', index=False)
    ...
    save_predictions(train_loader, 'train_predictions_23d_bpka.csv')
    save_predictions(val_loader, 'val_predictions_23d_bpka.csv')
    save_predictions(test_loader, 'test_predictions_23d_bpka.csv')

    # Save final model
    torch.save(model.pretrain_net.state_dict(), 'best_model_23d_bpka.pth')
...
def evaluate_model(model, loader, loss_fn, device):
    ...
    with torch.no_grad():
        for data in loader:
            data = data.to(device)
            out, _ = model(data.x, data.edge_index, data.edge_attr, data.batch)
            loss = loss_fn(out.squeeze(), data.y.float())  # Ensure labels are floats
...
# Modified evaluation function to return SMILES
def evaluate_model_with_metrics(model, loader, loss_fn, device):
    ...
    with torch.no_grad():
        for data in loader:
            ...
            # Process predictions
            preds = out.cpu().numpy()
            if preds.ndim == 0:  # Handle 0-dim array
                preds = np.array([preds])
            all_preds.extend(preds)

            # Process true values
            trues = data.y.cpu().numpy()
            if trues.ndim == 0:  # Handle 0-dim array
...
if __name__ == "__main__":
    # Data paths
    root = "/home/lxy/ToxPred_nitrification-main/data/bpka_23d"  # Dataset root directory
    filename = "bpka_30000.csv"  # CSV filename
