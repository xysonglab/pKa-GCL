...
from ATT3D_data import PkaDataset3D  # Replace with actual dataset class path
...
class ATT(torch.nn.Module):
    def __init__(self, n_heads=4, edge_dim=12, num_features=33, embedding_size=128, self_attention=False,
        ...
        self.out2 = Linear(embedding_size * 2, 1)  # Output a scalar value
...
def load_data(root, filename, batch_size=32, train_ratio=0.8, test_ratio=0.1):
    ...
    test_dataset = dataset[-test_size:]  # Last 10% (enforce last 10% as test set)
...
def train_model(model, train_loader, val_loader, test_loader, epochs=50, lr=0.001):
    ...
    for epoch in range(epochs):
        ...
        # Test phase (every 5 epochs or at final evaluation)
    ...
    pd.DataFrame(history).to_csv('training_log_3d.csv', index=False)
    ...
    save_predictions(train_loader, 'train_predictions_3d_pre.csv')
    save_predictions(val_loader, 'val_predictions_3d_pre.csv')
    save_predictions(test_loader, 'test_predictions_3d_pre.csv')

    # Save final model
    torch.save(model.pretrain_net.state_dict(), 'best_model_3d_pre.pth')
...
def evaluate_model(model, loader, loss_fn, device):
    ...
    with torch.no_grad():
        for data in loader:
            data = data.to(device)
            out, _ = model(data.x, data.edge_index, data.edge_attr, data.batch)
            loss = loss_fn(out.squeeze(), data.y.float())  # Ensure labels are float
...
if __name__ == "__main__":
    # Data paths
    root = "/home/lxy/ToxPred_nitrification-main/data/3d"  # Dataset root directory
