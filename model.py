...
class pKaMPNN_2D(nn.Layer):
    def __init__(self, node_in_dim, edge_in_dim, rbf_dim, hidden_dim, max_dist, \
        ...
        super(pKaMPNN_2D, self).__init__()
        self.num_conv = num_conv
        self.pool = pgl.nn.GraphPool(pool_type='max')
        self.geometry_embed = GraphEmbedding(rbf_dim, hidden_dim, max_dist, activation)
    ...
    def forward(self, a2a_graph, e2a_graph, e2e_graph):
        ...
        print("bond_feat", bond_feat.shape)
        print("dist_feat", dist_feat.shape)
        print("angle_feat", angle_feat.shape)
        ...
        # 使用 MaxPooling 对图级特征进行汇总
        graph_feat = self.pool(a2a_graph, node_feat)
        print("****graph_feat****", graph_feat.shape)

        return graph_feat


class pKaMPNN_3D(nn.Layer):
    def __init__(self, node_in_dim, rbf_dim, hidden_dim, cut_dist, \
        ...
        super(pKaMPNN_3D, self).__init__()
        self.num_conv = num_conv
        self.pool = pgl.nn.GraphPool(pool_type='max')  # 总和池化
        self.geometry_embed = GraphEmbedding(rbf_dim, hidden_dim, cut_dist, activation)
        self.node2edge_layers = nn.LayerList()
        self.edge2edge_layers = nn.LayerList()
        self.edge2node_layers = nn.LayerList()
        
        # 将 AttentivePooling 替换为 MaxPooling
        self.max_pool = pgl.nn.GraphPool(pool_type='max')
    ...
    def forward(self, a2a_graph, e2a_graph_list, e2e_graph_list):
        ...
        # Graph pooling with MaxPooling
        graph_feat = self.pool(a2a_graph, node_feat)  # 使用初始的总和池化
        graph_feat = self.max_pool(a2a_graph, node_feat)  # 使用 MaxPooling 替代 AttentivePooling
        return graph_feat



class pKaMPNN(nn.Layer):
    def __init__(self, node_in_dim, edge_in_dim, atom_in_dim, rbf_dim, hidden_dim, max_dist_2d, cut_dist_3d, mlp_dims, spa_weight, \
        ...
        super(pKaMPNN, self).__init__()
        self.spa_weight = spa_weight
        self.mpnn_2d = pKaMPNN_2D(node_in_dim, edge_in_dim, rbf_dim, hidden_dim, max_dist_2d, \
                                   num_conv, num_pool, dropout=0, dropout_pool=dropout_pool, activation=F.relu)
        self.mpnn_3d = pKaMPNN_3D(atom_in_dim, rbf_dim, hidden_dim, cut_dist_3d,  num_conv, \
...
class pKaGCL(nn.Layer):
    ...
    def forward(self, graph_2d, graph_3d,smiles):
        print("smiles",smiles.shape)
        feat_2d = self.mpnn_2d(*graph_2d)
        feat_3d = self.mpnn_3d(*graph_3d)
        feat_2d = self.trans_2d_layer(feat_2d)
        feat_3d = self.trans_3d_layer(feat_3d)
        print("*******************feat_2d*************",feat_2d.shape)#batch_sizexhidden_dim
