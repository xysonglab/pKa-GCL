...
def train(args, model, trn_loader, val_loader, output_dir_fold, result_dir_fold):
    ...
    for epoch in range(1, int(args.all_steps / epoch_step) + 1):
        ...
        if args.task == 'regression':
            ...
            if val_score < best_score_val:
                ...
                obj = {
                    ...
                    'optimizer': optim.state_dict(),  # 保存优化器状态
        ...
        log = f'Epoch: %d/{int(args.all_steps / epoch_step)}, loss: %.4f, Val {metric}: %.6f' % (
        epoch, trn_loss, val_score)
        log += f'time: %.2f, val_time: %.2f.\n' % (
    ...
    running_log += f'The best epoch is %d, best val {metric} is %.6f. \n' % (
    best_score_val, best_score)
    f = open(os.path.join(output_dir_fold, 'log.txt'), 'w')
    f.write(running_log)
    f.close()
...
if __name__ == "__main__":
    ...
    for idx in range(args.num_folds):
        trn_data_2d, val_data_2d = split_dataset(data_2d, idx, args.num_folds)
        print("*************************************")
        print(len(trn_data_2d))
        print(len(val_data_2d))
        print("*************************************")
        trn_data_3d, val_data_3d = split_dataset(data_3d, idx, args.num_folds)
        ...
        score = train(args, model, trn_loader, val_loader, output_dir_fold, result_dir_fold)
        scores.append(score)
