...
class DualDataLoader(object):
    ...
    def _generate_batch(self, indices):
        ...
        a2a_gs, e2a_gs, e2e_gs,smiles, labels = map(list, zip(*data_2d_batch))
        ...
        a2a_gs, e2a_gs, e2e_gs, smiles,labels = map(list, zip(*data_3d_batch))
        a2a_g = pgl.Graph.batch(a2a_gs).tensor()
        e2a_g = [pgl.BiGraph.batch([g[i] for g in e2a_gs]).tensor() for i in range(len(e2a_gs[0]))]

        #e2e_g=[]
        # for  i in range(len(e2e_gs[0])):
        #     for g,mol in zip(e2e_gs,smiles):
        #       #print("*********",mol)
        #       e2e_g.append(pgl.Graph.batch([g[i]]).tensor())
        # print("***********************************")
        # print(smiles)
        # print("***********************************")
        ...
        assert (y_2d - y_3d).sum().item() == 0
        return graph_2d, graph_3d, smiles,y_3d